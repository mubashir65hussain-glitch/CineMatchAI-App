...
# Inject custom layout styles forcing grid behavior on small viewports
st.markdown("""
    <style>
    /* CSS Grid Container that is responsive across screens */
    ...
    @media (max-width: 768px) {
        .responsive-movie-grid {
            grid-template-columns: repeat(2, 1fr) !important; /* Forces 2 movies per row on phones */
            gap: 12px !important;
        }
        .mtitle {
            font-size: 12px !important; /* Slightly smaller text so it wraps beautifully */
        }
        .moverview {
            display: none !important; /* Optional: hides long synopsis on mobile to look cleaner like Netflix */
        }
    }
    
    /* Ensure the detail buttons fill out the block nicely */
    .stButton > button {
...
""", unsafe_allow_html=True)
# Instead of native st.columns, let's open an HTML grid container
grid_html = '<div class="responsive-movie-grid">'
...
for i, (_, movie) in enumerate(all_movies.iterrows()):
    ...
    # Append each movie card item directly into the continuous HTML grid variable
    grid_html += f"""
    <div class="mcard" style="margin-bottom: 5px; display: flex; flex-direction: column; justify-content: space-between;">
        <div>
            {poster_html}
            <div class="minfo">
        ...
        <div style="padding: 0 14px 14px 14px;">
            <a href="{trailer if trailer else '#'}" target="_blank" style="text-decoration: none;">
                <button style="background: linear-gradient(135deg, #E50914 0%, #b20710 100%); color: white; border: none; padding: 10px 0; font-family: 'Raleway', sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; border-radius: 6px; width: 100%; box-shadow: 0 4px 15px rgba(229,9,20,0.3); cursor: pointer;">
                    Watch Trailer
                </button>
            </a>
        </div>
